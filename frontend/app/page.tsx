...
interface TranscriptEntry {
  ...
  startSeconds: number;
  endSeconds: number;
  speaker?: string; // New: speaker information
  words?: Array<{
    word: string;
    startTime: number;
    endTime: number;
...
export default function Home() {
  ...
  const [profanitySources, setProfanitySources] = useState<string[]>([]);
  ...
  const handleFileSelect = useCallback((response: UploadResponse) => {
    ...
    if (response.file.fileType === 'video' && response.videoInfo) {
      setVideoDuration(response.videoInfo.duration);
    } else if (response.file.fileType === 'audio' && response.audioInfo) {
      setVideoDuration(response.audioInfo.duration);
    }
  ...
  const getCurrentFileDuration = () => {
    if (isMultiVideoMode && videos.length > 0) {
      const video = videos[0];
      if (video.fileType === 'video' && video.videoInfo) {
        return video.videoInfo.duration;
      } else if (video.fileType === 'audio' && video.audioInfo) {
        return video.audioInfo.duration;
      }
    }
    
    if (uploadResponse) {
      if (uploadResponse.file.fileType === 'video' && uploadResponse.videoInfo) {
        return uploadResponse.videoInfo.duration;
      } else if (uploadResponse.file.fileType === 'audio' && uploadResponse.audioInfo) {
        return uploadResponse.audioInfo.duration;
      }
    }
  ...
  const handleGetTranscript = async () => {
    ...
    try {
      const result = await detectProfanity(
        fileId,
        sessionId,
        profanitySettings.customWords,
        setProcessingProgress
      );

      // Process the new response format
      const transcription = result.transcription;
      const profanitySegments = result.profanitySegments;
      
      // Set language detection info
      setDetectedLanguage(result.detectedLanguage || transcription.detectedLanguage || '');
      setLanguageConfidence(result.languageConfidence || transcription.languageConfidence || 0);
      setProfanitySources(result.processingInfo?.profanitySourcesUsed || []);
      
      // Convert transcription to the expected format
      let transcriptSegments: TranscriptEntry[] = [];
      
      if (transcription && transcription.segments && Array.isArray(transcription.segments)) {
        const words = transcription.words || [];
        let wordIdx = 0;
        
        transcriptSegments = transcription.segments.map((seg, idx) => {
          ...
          return {
            ...
            startSeconds,
            endSeconds,
            speaker: seg.speaker, // Add speaker information
            words: segWords.map(word => ({
              ...word,
              speaker: word.speaker // Add speaker information to words
            }))
          };
        });
      } else if (transcription && transcription.fullText) {
        const currentDuration = getCurrentFileDuration();
        transcriptSegments = [{
          index: 0,
  ...
  const handleMuteAndProcess = async () => {
    ...
    if (selectedWords.length === 0) {
      // Mute all profanity segments
      profanityData.forEach(segment => {
        segments.push({ start: segment.start, end: segment.end });
      });
    } else {
      // Mute ALL occurrences of each selected word in the transcript
      transcript.forEach(entry => {
        if (entry.words && Array.isArray(entry.words)) {
          entry.words.forEach((w: any) => {
            // Normalize both selected and transcript word
            const normWord = (w.word || '').replace(/[.,/#!$%^&*;:{}=\-_`~()"''"।!?]/g, '').toLowerCase();
            if (
              typeof w.startTime === 'number' &&
              typeof w.endTime === 'number' &&
              selectedWords.includes(normWord)
            ) {
              segments.push({ start: w.startTime, end: w.endTime });
            }
          });
        }
      });
      // Also include any profanity segments that match selected words (for robustness)
      profanityData.forEach(segment => {
        const normProfane = (segment.word || '').replace(/[.,/#!$%^&*;:{}=\-_`~()"''"।!?]/g, '').toLowerCase();
        if (selectedWords.includes(normProfane)) {
          segments.push({ start: segment.start, end: segment.end });
        }
      });
    }
    
    // Remove duplicate segments (by start/end)
    segments = segments
    ...
    try {
      ...
      setSelectedVersion(audioResult.version?.toString() || 'original');
      ...
      setProcessingError('Failed to process video');
      setIsProcessing(false);
      setProcessingProgress(null);
    }
  };
  ...
  const handleProcess = async () => {
    ...
    const segments = getActiveSegments().map(s => ({ start: s.start, end: s.end }));
    
    if (segments.length === 0) {
      setProcessingError('No segments selected for processing.');
      return;
    }
    ...
    try {
      ...
      setSelectedVersion(result.version?.toString() || 'original');
      setVersionHistoryRefreshKey(prev => prev + 1);
  ...
  const getTabIcon = (tab: string) => {
    switch (tab) {
      ...
      case 'profanity': return <Shield className="h-4 w-4" />;
      case 'audio-remove': return <VolumeX className="h-4 w-4" />;
      case 'trim': return <Scissors className="h-4 w-4" />;
      case 'multi-video': return <Video className="h-4 w-4" />;
  ...
  const getTabDescription = (tab: string) => {
    switch (tab) {
      case 'upload': return 'Upload your video files';
      case 'profanity': return 'Detect and filter profanity';
      case 'audio-remove': return 'Remove audio from specific segments';
      case 'trim': return 'Trim and cut video segments';
      case 'multi-video': return 'Edit multiple videos together';
      case 'history': return 'View edit history and versions';
      case 'settings': return 'Configure profanity detection';
      default: return '';
    }
  };
      ...
      <div className="container max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
            Professional video and audio editing with multi-file support, transcript analysis, custom profanity filtering, and version history. 
            Powered by Express.js backend with FFmpeg and AssemblyAI.
          </p>
          ...
          <TabsContent value="upload" className="space-y-6">
            <Card className="border-2 border-dashed border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Upload a single video or audio file for processing with FFmpeg and AssemblyAI
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="w-full">
                    ...
                    {!showTranscript && (
                      ...
                      <Button
                        ...
                        disabled={isProcessing || !getCurrentFileDuration()}
