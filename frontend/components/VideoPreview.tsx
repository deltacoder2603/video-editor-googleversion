...
import { UploadResponse, downloadVideo, formatFileSize, formatDuration } from '@/lib/api';
...
export default function VideoPreview({ 
  ...
  const videoUrl = processedVideoUrl || (uploadResponse ? 
    `http://localhost:3001/uploads/${uploadResponse.file.filename}` : null);
      ...
      {uploadResponse && (
        <div className="absolute top-4 left-4 bg-black/70 text-white px-3 py-1 rounded-lg text-xs z-10">
          <div className="flex items-center space-x-2">
            ...
            <span>{formatDuration(uploadResponse.videoInfo.duration)}</span>
          </div>
        </div>
      )}
