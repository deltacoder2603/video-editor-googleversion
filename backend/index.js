...
async function extractAudioForTranscription(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .output(outputPath)
      .noVideo()
      .audioCodec('flac') // FLAC is recommended for Google Cloud Speech
...
async function transcribeAudio(audioPath, languageCode = 'en-US') {
  try {
    ...
    // Configure the audio settings and recognition config
    const audio = {
    ...
    if (audioSizeInMB > 10) { // Use long-running operation for larger files
      const [operation] = await speechClient.longRunningRecognize(request);
      const [response] = await operation.promise();
      return processTranscriptionResponse(response);
      } else {
      // Use regular recognize for smaller files
  ...
  if (response.results && response.results.length > 0) {
    response.results.forEach((result, index) => {
      if (result.alternatives && result.alternatives.length > 0) {
        ...
        transcription.fullText += alternative.transcript + ' ';
        ...
        if (alternative.words) {
          alternative.words.forEach(word => {
            transcription.words.push({
              word: word.word,
              startTime: word.startTime ? parseFloat(word.startTime.seconds || 0) + (word.startTime.nanos || 0) / 1000000000 : 0,
              endTime: word.endTime ? parseFloat(word.endTime.seconds || 0) + (word.endTime.nanos || 0) / 1000000000 : 0,
...
function findProfanityInTranscription(transcription, profanityWords) {
  const profanitySegments = [];
  
  transcription.words.forEach(word => {
    if (profanityWords.has(word.word.toLowerCase())) {
...
app.post('/api/transcribe', async (req, res) => {
  const { fileId, sessionId, languageCode = 'en-US' } = req.body;
  ...
  try {
    ...
    const audioPath = path.join(tempDir, `${fileId}_audio.flac`);
...
app.post('/api/detect-profanity', async (req, res) => {
  const { fileId, sessionId, customWords = [], languageCode = 'en-US' } = req.body;
  console.log('Detect Profanity called with:', { sessionId, fileId, languageCode, customWords });
  console.log('Current sessions:', Array.from(videoSessions.keys()));
  try {
    const session = videoSessions.get(sessionId);
    if (!session) {
    ...
    // Combine default profanity list with custom words
    const profanityWords = new Set([...customProfanityList, ...customWords.map(w => w.toLowerCase())]);
    
    // Find profanity in transcription
    const profanitySegments = findProfanityInTranscription(transcription, profanityWords);
